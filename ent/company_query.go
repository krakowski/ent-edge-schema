...
func (cq *CompanyQuery) QueryAddress() *AddressQuery {
...
func (cq *CompanyQuery) QueryCompanyAddresses() *CompanyAddressQuery {
	query := (&CompanyAddressClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CompanyQuery) Clone() *CompanyQuery {
	...
	return &CompanyQuery{
		...
		withCompanyAddresses: cq.withCompanyAddresses.Clone(),
...
func (cq *CompanyQuery) WithAddress(opts ...func(*AddressQuery)) *CompanyQuery {
	query := (&AddressClient{config: cq.config}).Query()
...
func (cq *CompanyQuery) WithCompanyAddresses(opts ...func(*CompanyAddressQuery)) *CompanyQuery {
	query := (&CompanyAddressClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withCompanyAddresses = query
	return cq
}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	if query := cq.withAddress; query != nil {
		if err := cq.loadAddress(ctx, query, nodes,
			func(n *Company) { n.Edges.Address = []*Address{} },
			func(n *Company, e *Address) { n.Edges.Address = append(n.Edges.Address, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withCompanyAddresses; query != nil {
		if err := cq.loadCompanyAddresses(ctx, query, nodes,
			func(n *Company) { n.Edges.CompanyAddresses = []*CompanyAddress{} },
			func(n *Company, e *CompanyAddress) { n.Edges.CompanyAddresses = append(n.Edges.CompanyAddresses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CompanyQuery) loadAddress(ctx context.Context, query *AddressQuery, nodes []*Company, init func(*Company), assign func(*Company, *Address)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(company.AddressTable)
		s.Join(joinT).On(s.C(address.FieldID), joinT.C(company.AddressPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(company.AddressPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(company.AddressPrimaryKey[1]))
...
func (cq *CompanyQuery) loadCompanyAddresses(ctx context.Context, query *CompanyAddressQuery, nodes []*Company, init func(*Company), assign func(*Company, *CompanyAddress)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(companyaddress.FieldCompanyID)
	}
	query.Where(predicate.CompanyAddress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.CompanyAddressesColumn), fks...))
	}))
