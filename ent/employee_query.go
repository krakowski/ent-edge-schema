...
func (eq *EmployeeQuery) QueryAddress() *AddressQuery {
	query := (&AddressClient{config: eq.config}).Query()
...
func (eq *EmployeeQuery) QueryEmployeeAddresses() *EmployeeAddressQuery {
	query := (&EmployeeAddressClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmployeeQuery) Clone() *EmployeeQuery {
	...
	return &EmployeeQuery{
		...
		withAddress:           eq.withAddress.Clone(),
		withEmployeeAddresses: eq.withEmployeeAddresses.Clone(),
...
func (eq *EmployeeQuery) WithAddress(opts ...func(*AddressQuery)) *EmployeeQuery {
	query := (&AddressClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withAddress = query
	return eq
}
...
func (eq *EmployeeQuery) WithEmployeeAddresses(opts ...func(*EmployeeAddressQuery)) *EmployeeQuery {
	query := (&EmployeeAddressClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withEmployeeAddresses = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	...
	if query := eq.withAddress; query != nil {
		if err := eq.loadAddress(ctx, query, nodes,
			func(n *Employee) { n.Edges.Address = []*Address{} },
			func(n *Employee, e *Address) { n.Edges.Address = append(n.Edges.Address, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withEmployeeAddresses; query != nil {
		if err := eq.loadEmployeeAddresses(ctx, query, nodes,
			func(n *Employee) { n.Edges.EmployeeAddresses = []*EmployeeAddress{} },
			func(n *Employee, e *EmployeeAddress) {
				n.Edges.EmployeeAddresses = append(n.Edges.EmployeeAddresses, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (eq *EmployeeQuery) loadAddress(ctx context.Context, query *AddressQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Address)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(employee.AddressTable)
		s.Join(joinT).On(s.C(address.FieldID), joinT.C(employee.AddressPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(employee.AddressPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(employee.AddressPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "address" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (eq *EmployeeQuery) loadEmployeeAddresses(ctx context.Context, query *EmployeeAddressQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *EmployeeAddress)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(employeeaddress.FieldEmployeeID)
	}
	query.Where(predicate.EmployeeAddress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.EmployeeAddressesColumn), fks...))
	}))
